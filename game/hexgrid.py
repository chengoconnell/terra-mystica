...
class HexGrid(Generic[T]):
    ...
    - getting/setting values at hex positions
    - checking if a hex position exists in the grid
    - iterating over all positions and values
    - finding neighbors of a hex position
    - calculating distance between hex positions
    - finding all hexes within a given range
    - pathfinding between positions

    The grid uses axial coordinates (q, r) with flyweight pattern
    and can store any type T at each position."""
    ...
    def get(self, coord: HexCoord) -> T:
        ...
        :raises KeyError: if the position is empty
        """
        try:
    ...
    def get_filled_neighbors(self, coord: HexCoord) -> list[tuple[HexCoord, T]]:
        ...
        TYPE: Tuple return type for rich data
        """
        result = []
    ...
    def find_path(
        ...
        TYPE: Optional callable parameter, union return type
    ...
    def __contains__(self, coord: HexCoord) -> bool:
        ...
        TYPE: Protocol method for container protocol
        """
    ...
    def get_bounds(self) -> tuple[HexCoord, HexCoord]:
        ...
        :return: (min_corner, max_corner) as hex coordinates
